...
```
#Downloading the selected high-NRCD samples from ENA
...
### 2. Quality Control
The analysis involved pairwise comparison of the 2 groups. Quality Control was carried out using Fastqc (v 0.12.1). Poor QC can lead to biological conclusions derived from technical artifacts. Trimming the low quality reads can lead to inflated species counts and false diversity measures, and adapter contamination can cause false positives as taxa will be incorrectly placed between groups. 
```
...
`--min S` - --min S — tells kraken-biom to record reads assigned below the minimum rank at the specified minimum rank (S = species).
```
kraken-biom \
